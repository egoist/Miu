...
CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
  ...
  function blockNormal(stream, state) {
    ...
    if (state.indentationDiff >= 4) {
      ...
      return null;
    } else if (stream.eatsub(1) === '#' || (prevLineHasContent && stream.match(headerRE)) || stream.eatsub(2) === '~~') {
      state.header = true;
      if(stream.eatsub(1) === '#') state.headerType = header;
      if(stream.eatsub(2) === '##') state.headerType = header2;
      else if(stream.eatsub(2) === '~~') state.headerType = del;
